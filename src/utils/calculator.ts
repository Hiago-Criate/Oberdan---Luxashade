...
function getMultiplicador(cod: string) {
  return (cod.includes('05C') || cod.includes('05L')) ? 1.2 : 1.1;
}
...
function calcularValorComponente(comp: any, giCod: string, larguraMM: number) {
  const larguraMetros = larguraMM / 1000;
  const quantidade = calcularQuantidade(comp.f, larguraMM, comp.q);
  
  if (comp.p === 'SNN') return 0;
...
export function calculatePrice(data: any) {
  ...
  const motorPrice = MOTOR_PRICES[motor]?.[railColor] || 0;

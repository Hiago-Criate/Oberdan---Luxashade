...
`marcas, acionamentos, familias, modelos, tipos_tecido, colecoes, cores_tecido, cores_acabamento, estoque_tecido, produtos (8015, preço m² + larg_max override), modelo_limites (45), motores/motor_modelos, trilho_modelos/trilho_produtos/trilho_componentes, opcionais/opcional_modelos/opcional_exclusoes, emissores, app_config, audit_log`.

### Motores por grupo (coluna "SHADES" do painel)
Cada motor define **em quais grupos de produto (modelos)** pode ser usado, via `motor_modelos(motor_id, modelo)`. "Grupo de produto" = o **prefixo do código** (ex.: 400 = ROLLER SHADE LX55 MOTORIZADA, 402 = LX60 MOTORIZADA). No `get_catalog`, cada motor traz `modelos[]` (+ `familias[]` derivado p/ compat). No app, `motorsForModelo(modelo, familia)` (em `motorPrices.ts`) lista, para o **modelo** escolhido, só os motores liberados. No painel (módulo **Motores**), a coluna **SHADES (grupos)** abre um editor (modal) família→modelos motorizados, com o **código do grupo** (400, 402…) no rótulo e no resumo. RPC auxiliar `modelo_grupos()` lista (família, modelo, grupo, motorizado). Trilho continua separado (`uso_trilho`).

Larg. máx por tecido: `produtos.larg_max` (NULL = herda do modelo) limita a largura quando o rolo do tecido é mais estreito; validado no app ao escolher a Cor do Tecido (`fabricMaxWidth` + reason `LARGURA_TECIDO`); editável no módulo **Preços** (coluna Larg. máx + ação em massa).

### Opcionais por grupo (deduplicados)
Os acessórios ShadeXP foram **deduplicados por código** (cada acessório uma vez em `opcionais`, ligado a N modelos via `opcional_modelos`; exclusões TDF↔bandôs mescladas). O vínculo é **por modelo** (precisão: ex. Guia 55mm nos ROLÔ menores, Guia 80mm nos maiores). No painel (módulo **Opcionais**), a coluna **Grupos / modelos** abre um editor (modal) organizado por família → modelos (com "marcar todos" por grupo). `get_catalog.opcionais_por_modelo` continua chaveado por modelo — o app não muda.

### Largura máxima por tecido (override por produto)
`produtos.larg_max` (nullable) permite uma **largura máx menor que a do modelo** para tecidos de rolo mais estreito. Vai na tupla do produto no `get_catalog` (10º campo). No app, `fabricMaxWidth(modelo, coleção, cor)` (catalogStore) dá o menor override da combinação; como a **largura é digitada antes do tecido**, a checagem acontece no passo **Cor do Tecido** (aviso vermelho) e no `calculatorShade` (`reason: 'LARGURA_TECIDO'` → bloqueia adicionar). No painel **Preços**, coluna **Larg. máx (mm)** por produto (placeholder = padrão do modelo; vazio = herda) + ação em massa (definir / voltar ao padrão nos filtrados).

### Emissores (categoria EMISSORES no app)
Entidade de primeira classe (tabela `emissores`): `codigo, descricao, valor, canais, motor_brand (texto, ex SOMFY/IVOLVE/…), brand_luxashade, brand_shadexp, ativo, ordem`. O `get_catalog` devolve `emissores` (com `brands` derivado dos dois booleanos). No app, `src/data/emissores.ts` lê esses campos (sem heurística): o fluxo `EmissorOrderFlow` monta os botões de **marca do motor dinamicamente** (`motorBrandsFor`) e lista os emissores por (marca da cortina × marca do motor). Cadastro/edição no módulo **Emissores** do painel. (Antes eram os `opcionais` tipo `controle_motorizada`, agora migrados para cá.) RLS: **leitura pública, escrita só autenticada**. Triggers de `updated_at` e auditoria.
